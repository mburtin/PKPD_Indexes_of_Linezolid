...
bigelow_full <- bind_rows(bigelow_10mg, bigelow_30mg, bigelow_100mg, bigelow_335mg) |>
  rename(time = x, C_CENTRAL = y) |>
  as_tibble()

rm(bigelow_10mg, bigelow_30mg, bigelow_100mg, bigelow_335mg)
```
...
ggplot() +
    ...
    geom_line(data = bigelow_full, aes(x = time, y = C_CENTRAL, color = factor(ID)), linetype = "dashed") +
...
ev05 <- ev(ID = 800, amt = 80, cmt ="A")
ev06 <- ev(ID = 400, amt = 40, cmt = "A")
ev07 <- ev(ID = 200, amt = 20, cmt = "A")
andes_dose <- rbind(ev05@data, ev06@data, ev07@data)
...
ggplot() +
    ...
    geom_point(data = sandberg_data, aes(x = time, y = conc, color = factor(ID)), shape = 2) +
