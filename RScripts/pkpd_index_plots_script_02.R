...
PKPD_target_plots_600mg <- function(obs_data, pred_data, effect, auc_target, tmic_target, title) {
  ...
  ggplot(obs_data |> filter(PKPD_Index %in% c("I2_AUC", "I3_ToverMIC")), aes(x = value, y = deltaLog10CFU)) +
    ...
    facet_wrap(~PKPD_Index,
      scales = "free_x", strip.position = "bottom", nrow = 1,
      labeller = labeller(
        PKPD_Index = c(
          "I2_AUC" = "fAUC/MIC",
          "I3_ToverMIC" = "fT>MIC (%)"
    ...
    facetted_pos_scales(
      x = list(
        PKPD_Index == "I2_AUC" ~ scale_x_continuous(
          limits = c(1, 1000),
          breaks = c(1, 10, 100, 1000),
          trans = "log10",
          name = expression(italic(f) * AUC / MIC)
        ),
        PKPD_Index == "I3_ToverMIC" ~ scale_x_continuous(
          limits = c(0, 100),
    ...
    geom_text(
      data = obs_data |> filter(PKPD_Index == "I3_ToverMIC"),
      aes(x = 85, y = 4, label = paste(round(tmic_atteignment, 0), "%")), size = 4, hjust = 0
    ) +
    geom_text(
      data = obs_data |> filter(PKPD_Index == "I2_AUC"),
