...
clean_sim_table <- function(df) {
  df |>
    ...
    select(
      STRN, MIC, DoseGroup, ID, AMT, time, B0, Log10CFU_CSF,
...
sim_formated_data <- bind_rows(fractioned_24h_full, continuous_inf_24h, control_24h) |>
  ...
  pivot_longer(
    cols = c(
      CENTRAL_Cmax, CENTRAL_AUC_MIC, CENTRAL_ToverMIC,
  ...
  select(-c(
    Cmax_CENTRAL, AUCCENTRAL, TOVER_MIC_CENTRAL,
    Cmax_CSF, AUCCSF, TOVER_MIC_CSF
  )) |>
  ungroup()

# Split sim_formated_data for each target, and reorder columns
csf_sim_data <- sim_formated_data |>
  rename("Log10CFU" = Log10CFU_CSF) |>
  filter(PKPD_Index %in% c("CSF_Cmax", "CSF_AUC_MIC", "CSF_ToverMIC"))
mix_sim_data <- sim_formated_data |>
  rename("Log10CFU" = Log10CFU_CSF) |>
  filter(PKPD_Index %in% c("CENTRAL_Cmax", "CENTRAL_AUC_MIC", "CENTRAL_ToverMIC"))
...
sim_results_fractioned <- list(
  fractioned_results = fractioned_results,
  fractioned_24h_full = fractioned_24h_full
)

sim_results_continuous_inf <- list(
  continuous_inf_results = continuous_inf_results,
  continuous_inf_24h = continuous_inf_24h,
  continuous_inf_amt = continuous_inf_amt
)

sim_results_control <- list(
...
rm(
  fractioned_results, fractioned_24h_full,
