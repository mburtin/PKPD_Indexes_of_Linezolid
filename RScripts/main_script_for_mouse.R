...
simulate_fractioned_dose <- function(i, fractioned_daily_AMT) {
  ...
  q12 <- model_list[[i]] |>
    ...
    mrgsolve::ev(amt = fractioned_daily_AMT / 2, ii = 12, addl = 1) |>
  ...
  q6 <- model_list[[i]] |>
    mrgsolve::idata_set(i_data) |>
    mrgsolve::ev(amt = fractioned_daily_AMT / 4, ii = 6, addl = 3) |>
    mrgsolve::mrgsim(delta = 0.1, end = 24) |>
...
sim_data <- sim_formated_data |> filter(PKPD_Index %in% c("CENTRAL_Cmax", "CENTRAL_AUC_MIC", "CENTRAL_ToverMIC"))
