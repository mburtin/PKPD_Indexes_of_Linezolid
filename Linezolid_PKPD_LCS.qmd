...
csf_plot <- PKPD_index_plots(obs_data = csf_data[["obs_mean"]], 
                             ...
                             filename = "CSF-derived PKPD Index for Linezolid.png")
...
mix_plot <- PKPD_index_plots(obs_data = csf_with_plasma_index_data[["obs_mean"]], 
                             ...
                             filename = "Plasma-derived PKPD Index for Linezolid.png")
...
```{r}
## Execute the VPC script
source("RScripts/vpc_script.R")

## Allow to setup your VPC data quickly
...
p1 <- ggplot() +
  ...
  geom_text(aes(x = Inf, y = Inf, 
                label = paste("Adj-R² of obs median = ", round(csf_data$correlation_data |> filter(PKPD_Index == "CSF_Cmax") |> pull(Adj.Rsq), 2))),
            hjust = 1.1, vjust = 3, size = 2.5, fontface = "italic", color = "black") +
  theme_minimal() +
...
p2 <- ggplot() +
  ...
  geom_text(aes(x = Inf, y = Inf, 
                label = paste("Adj-R² of obs median = ", round(csf_data$correlation_data |> filter(PKPD_Index == "CSF_AUC_MIC") |> pull(Adj.Rsq), 2))),
            hjust = 1.1, vjust = 3, size = 2.5, fontface = "italic", color = "black") +
  theme_minimal() +
...
p3 <- ggplot() +
  ...
  labs(x = "T>MIC (%)", y = NULL) +
  geom_text(aes(x = Inf, y = Inf, 
                label = paste("Adj-R² of obs median = ", round(csf_data$correlation_data |> filter(PKPD_Index == "CSF_ToverMIC") |> pull(Adj.Rsq), 2))),
            hjust = 1.1, vjust = 3, size = 2.5, fontface = "italic", color = "black") +
  theme_minimal() +
  theme(axis.text.y = element_blank(),
        plot.margin = margin(0, 0, 0, 0),
...
ggsave(file.path(getwd(), "results", "VPC - CSF.png"), width = 9, height = 3)
