...
csf_plot <- generate_PKPD_plots(obs_data = csf_data[["sim_data"]] |> 
...
csf_plot2 <- generate_PKPD_plots2(obs_data = csf_data[["sim_data"]] |> 
                                  filter(DoseGroup == "fractioned_12h" & AMT == 1200), 
                                pred_data = csf_data[["pred_data"]], 
                                title = "LNZ PKPD Index for CSF" ,
                                filename = "LNZ PKPD Indexes for CSF target.png")
...
ggsave("testt2.png", width = 8, height = 4, dpi = 1200)
...
p1 <- ggplot() +
  ...
  scale_linetype_manual(name = "Percentiles from PKPD model",
                        values = c("p5_obs" = "dashed", "p50_obs" = "solid", "p95_obs" = "dashed"),
                        labels = c("p5_obs" = "5%", "p50_obs" = "50%", "p95_obs" = "95%")) +
  scale_color_manual(name = "Percentiles from Emax model",
  ...
  geom_text(aes(x = Inf, y = Inf, 
                label = paste("R² = ", round(csf_data$correlation_data |> filter(PKPD_Index == "CSF_Cmax") |> pull(Adj.Rsq), 2))),
...
ggsave(file.path(getwd(), "results", "VPC - CSF.png"), width = 9, height = 3, dpi = 1200)
