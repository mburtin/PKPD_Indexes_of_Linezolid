...
# Plots parameters
plot_title = "Mouse study - Linezolid PK/PD index - Mixed PBPK effect and linear CL Indices"
plot_filename = "Andes - PKPD_index - Mixed PBPK effect and linear CL Indices.png"

###############################################################
##             Monocomp non linear model                     ##
###############################################################

# Load the model
model_file_linear <- mread("model/andes/Monocomp_IV_Linear_CL")

# Specific model parameters for each strains
model_list <- list(
    # S. Aureus
    SA01943 = param(model_file_linear, B0 = 6.3, Bmax = 9.26, Kg = 1.09,
                    Emax = 1.93, EC50 = 1.1, Kon = 0.016, MIC = 2.0)
)

# Execute the main script
source("RScripts/mouse_script_for_linear_mixed.R")
...
# Load the model
model_file_pbpk <- mread("model/andes/Bicomp_PBPK_NonLinear_CL")

# Specific model parameters for each strains
model_list <- list(
    # S. Aureus
    SA01943 = param(model_file_pbpk, B0 = 6.3, Bmax = 9.26, Kg = 1.09,
                    Emax = 1.93, EC50 = 1.1, Kon = 0.016, MIC = 2.0)
)

# Execute the main script
source("RScripts/mouse_script_for_pbpk_mixed.R")

###############################################################
##                     Post data treatment                   ##
###############################################################

# Merge PKPD Indices of linear model with the effects of PBPK model
...
# Format the data to recover deltaLog10CFU and Rsq
...
# Generate predictions to plot the correlation curve
...
# Generate the PK/PD Indices plots
