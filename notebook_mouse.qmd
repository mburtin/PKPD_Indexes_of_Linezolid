...
# Load the model
model_file <- mread("model/Mouse_model_bicomp_non_linear")
...
pkpd_plot <- PKPD_index_plots(obs_data = data[["sim_data"]], 
                             ...
                             title = "Reproduction of Sandberg PKPD Index for Linezolid - Bicomp model" ,
                             file_name = "Sandberg PKPD Index for Linezolid - Bicomp model.png")
```

# Differents PK Trials

Bioavailibility analysis in Bigelow study
```{r}
library(minpack.lm)
...
```{r}
library(tidyverse)
# Extracted data from Bigelow study
...
# Sandberg PK
```{r}
# Extracted data from Sandberg study
...
ggplot() +
    ...
    labs(title = "Plasmatic free-concentration of Linezolid in mouse - Sandberg",
         x = "Time (h)",
         y = "Concentration (mg/L)",
         color = "Dose (mg/kg)") +
    theme_minimal()
...
# Andes PK
```{r}
# Extracted data from Andes study
